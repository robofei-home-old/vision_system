...
def predict_zed(cfg=DEFAULT_CFG):
    ...
    predictor = DetectionPredictor(overrides={'model': cfg.model})
    ...
    while True:
        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            left_image = zed.retrieve_image(sl.VIEW.LEFT).get_data()
            left_image = cv2.cvtColor(left_image, cv2.COLOR_BGR2RGB)

            # Call the `predict_cli` method of the `DetectionPredictor` class with the modified `args` dictionary
            args = {'model': cfg.model, 'source': source}
            predictor.args = args
            predictor.predict_cli()

            # Display the annotated image
            cv2.imshow("Annotated Image", left_image)
